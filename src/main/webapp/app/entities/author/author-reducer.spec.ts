...
import reducer, { createEntity, deleteEntity, getEntities, getEntity, partialUpdateEntity, reset, updateEntity } from './author.reducer';
import { EntityState } from 'app/shared/reducers/reducer.utils';
import { defaultValue, IAuthor } from 'app/shared/model/author.model';
...
describe('Entities reducer tests', () => {
  ...
  describe('Actions', () => {
    ...
    it('dispatches FETCH_AUTHOR_LIST actions', async () => {
    ...
    it('dispatches FETCH_AUTHOR actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches CREATE_AUTHOR actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches UPDATE_AUTHOR actions', async () => {
      const expectedActions = [
        {
    ...
    it('dispatches PARTIAL_UPDATE_AUTHOR actions', async () => {
      const expectedActions = [
        {
      ...
      await store.dispatch(partialUpdateEntity({ id: 'ABC' }));
      expect(store.getActions()[0]).toMatchObject(expectedActions[0]);
      expect(store.getActions()[1]).toMatchObject(expectedActions[1]);
      expect(store.getActions()[2]).toMatchObject(expectedActions[2]);
    });

    it('dispatches DELETE_AUTHOR actions', async () => {
      const expectedActions = [
        {
